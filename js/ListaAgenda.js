...
function OutlookButtonClick(e) {
    var idExportar = e.target.parentNode.attributes.eventoid.value;
...
function excluirEventoDaListEnviarLembrete(e) {
    ...
    var splitCookieEvento = docCookies.getItem('CookieListaAgendaIguatemi').split(',');
    
    splitCookieEvento = $.grep(splitCookieEvento, function (value) {
        return value != excluirId;        
    });
    docCookies.setItem('CookieListaAgendaIguatemi', splitCookieEvento);
        
    var splitCookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaIguatemi").split(',');
    ...
    for (var i = 0; i < splitCookieLembrete.length; i++) {
        if (splitCookieLembrete[i].split('&D')[0] == excluirId) {
            ...
            docCookies.setItem('CookieEnviarLembreteAgendaIguatemi', splitCookieLembrete);
        }
    }
...
function addEventoAListEnviarLembrete(e) {
    ...
    if (e.currentTarget.checked == true) {
        ...
        var cookieEvento = docCookies.getItem('CookieListaAgendaIguatemi');
        ...
        if (typeof cookieEvento != 'undefined' && cookieEvento != "" && cookieEvento != null) { // Se cookie já tiver valor
            ...
            if ($.inArray(idEvento, splitCookieEvento) == -1) // Se idEvento NÃO estiver no cookie
            {
                ...
                var cookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaIguatemi");  //  Adiciona o cookie 
                ...
                docCookies.setItem("CookieEnviarLembreteAgendaIguatemi", splitCookieLembrete);  //  evento
            ...
            docCookies.setItem("CookieListaAgendaIguatemi", idEvento);
            docCookies.setItem("CookieEnviarLembreteAgendaIguatemi", idEvento + '&D' + diasParaLembrete);
...
function MarcarLembrete() {
    ...
    var getCookieLembrete = docCookies.getItem('CookieEnviarLembreteAgendaIguatemi').split(',');
